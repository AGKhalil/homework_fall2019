...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards for i in indices]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            discounts = [np.power(self.gamma, i - start_time_index) for i in indices]
            ...
            discounted_rtg = [discounts[i] * rewards[i] for i in indices]

            # 4) calculate a scalar: sum_{t'=t}^{T-1} gamma^(t'-t) * r_{t'}
            sum_discounted_rtg = np.sum(discounted_rewards)
